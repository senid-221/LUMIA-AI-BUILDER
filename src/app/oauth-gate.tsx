    ...
    <main className="oauth-page">
      <div className="oauth-card">
        ...
        <div className="oauth-actions">
          <a className="oauth-button google" href="/api/auth/oauth/google"><span className="oauth-icon">G</span><span>Continue with Google</span></a>
          <a className="oauth-button github" href="/api/auth/oauth/github"><span className="oauth-icon">●</span><span>Continue with GitHub</span></a>
        </div>
        {error && <p className="oauth-error">{error}</p>}
        <p className="oauth-note">By continuing, you agree to use Lumia Builder responsibly.</p>
      </div>
      <style jsx global>{`
        ...
        .oauth-button{height:52px;border-radius:14px;text-decoration:none;display:flex;align-items:center;justify-content:center;gap:11px;font-size:15px;font-weight:750;transition:.18s}
        .oauth-button:hover{transform:translateY(-1px);box-shadow:0 9px 22px rgba(15,23,42,.1)}
        .oauth-button.google{background:#fff;color:#111827;border:1px solid #d1d5db}
        .oauth-button.github{background:#111827;color:#fff;border:1px solid #111827}
        .oauth-icon{width:22px;font-weight:900;font-size:17px}
