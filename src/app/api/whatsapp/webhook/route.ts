...
export async function POST(request: Request) {
  try {
    ...
    const result = await nvidiaChat([
      { role: "system", content: "You are Lumia AI WhatsApp Agent. Answer clearly and helpfully. Keep responses concise enough for WhatsApp." },
    ...
    const response = await fetch(`https://graph.facebook.com/v23.0/${encodeURIComponent(phoneNumberId)}/messages`, {
      ...
      body: JSON.stringify({ messaging_product: "whatsapp", to: from, type: "text", text: { body: result.content } }),
      cache: "no-store",
    });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) return NextResponse.json({ ok: false, error: data?.error?.message || `WhatsApp API error ${response.status}` }, { status: 502 });
    return NextResponse.json({ ok: true, delivered: true, messageId: data?.messages?.[0]?.id || null });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "WhatsApp webhook failed." }, { status: 500 });
  }
}
