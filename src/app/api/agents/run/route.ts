...
export async function POST(request: Request) {
  try {
    ...
    if (!prompt || !projectId) {
      return NextResponse.json(
        { ok: false, error: "prompt and projectId are required." },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json({
      ...
      memoryCount: team.memoryCount,
    });
  } catch (error) {
    console.error("Agent team run failed:", error);

    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Agent team failed.",
      },
      { status: 500 },
    );
  }
