...
export async function POST(request: Request) {
  try {
    ...
    await prisma.builderProject.update({ where: { id: jobProjectId }, data: { status: "BUILDING", description: prompt } });
    ...
    const buildRequest = team.context
    ...
    if (built.files?.length) {
      await prisma.$transaction(
        built.files.map((file: any) =>
          prisma.builderFile.upsert({
    ...
    const finalStatus = built.result?.status === "PASSED" ? "READY" : "FAILED";
    ...
    await prisma.builderTask.create({
      data: {
        ...
        status: finalStatus === "READY" ? "DONE" : "FAILED",
        input: { prompt },
        output: { plan: planned.plan, teamSteps: team.steps, retries: built.retries, memoryCount: planned.memoryCount, result: built.result },
      },
    });

    await prisma.builderProject.update({ where: { id: jobProjectId }, data: { status: finalStatus } });
    ...
    return NextResponse.json({
      ...
      plan: planned.plan,
      team,
      memoryCount: planned.memoryCount,
      error: finalStatus === "FAILED" ? built.result?.reason || "Build failed." : undefined,
    }, { status: finalStatus === "READY" ? 200 : 500 });
  } catch (error) {
    console.error(`[LUMIA BUILD] request failed`, error);
    const message = error instanceof Error ? error.message : "Unable to complete build.";
